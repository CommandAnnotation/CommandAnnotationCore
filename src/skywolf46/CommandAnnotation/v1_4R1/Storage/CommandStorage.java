package skywolf46.CommandAnnotation.v1_4R1.Storage;
...
import skywolf46.CommandAnnotation.v1_4R1.Data.CommandActionWrapper;
import skywolf46.CommandAnnotation.v1_4R1.Data.CommandArgument;
import skywolf46.CommandAnnotation.v1_4R1.Data.CommandConditionWrapper;
...
public class CommandStorage {
    ...
    private CommandConditionWrapper ccw = new CommandConditionWrapper();
    ...
    private void registerAutoCommand(String mainCommand, String[] split, MCReflectiveAutoCommand action) {
        // Recursive register completed!
        if (split.length == 0) {
            if (this.ccw == null)
                ccw = new CommandConditionWrapper();
            ccw.addAutoCommand(action);
...
//            this.fallBackOnSubcommandNotExist = fallBackOnSubcommandNotExist;
        ...
        storage.computeIfAbsent(split[0], key -> new CommandStorage(mainCommand, currentIndex + 1))
                .registerAutoCommand(
                        mainCommand, Arrays.copyOfRange(split, 1, split.length), action
                );
    }
...
//
//    private void activeAutoWrapper(String[] arg) {
//        if (arg.length == 0)
//            return this.action;
